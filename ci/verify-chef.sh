...
if [ "x$ACCEPTANCE" != "x" ]; then
  # Skip if acceptance tests do not exist.
  if [ -d /opt/$PROJECT_NAME/embedded/lib/ruby/gems/*/gems/chef-[0-9]*/acceptance ]; then
    # Test against the vendored Chef gem
    cd /opt/$PROJECT_NAME/embedded/lib/ruby/gems/*/gems/chef-[0-9]*/acceptance
    sudo env PATH=$PATH AWS_SSH_KEY_ID=$AWS_SSH_KEY_ID bundle install --without development
    sudo env PATH=$PATH AWS_SSH_KEY_ID=$AWS_SSH_KEY_ID bundle exec chef-acceptance test gem_versions
  fi
else
  # Test against the vendored Chef gem
